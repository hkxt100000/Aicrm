            ...
            if statement and ('CREATE TABLE' in statement or 'CREATE INDEX' in statement):
                try:
                    ...
                    if 'already exists' not in str(e):
                        print(f"[警告] SQL执行出错: {str(e)[:100]}")
    print("[数据库] 源数据管理表初始化完成")
...
def check_token(
    request: Request,
    token: Optional[str] = Query(None, alias="api_token")
):
    """
    验证 API Token
    支持两种方式：
    1. Query 参数：?api_token=xxx (旧方式，兼容)
    2. Authorization Header：Bearer xxx (新方式，推荐)
    """
    # 方式1：从 Query 参数获取（旧 API 兼容）
    if token and token == API_TOKEN:
        return token
    
    # 方式2：从 Authorization Header 获取（新认证系统）
    auth_header = request.headers.get('Authorization', '')
    if auth_header.startswith('Bearer '):
        # 这是新的认证系统，已通过中间件验证
        # request.state.user 已包含用户信息
        if hasattr(request.state, 'user') and request.state.user:
            return 'authenticated'  # 返回任意非空值表示已认证
...
async def get_customers(
    ...
    # 数据隔离：非超级管理员只能看到自己的客户
    # 如果 current_user 为 None（使用旧API Token），跳过数据隔离（向后兼容）
    if current_user and not current_user.get('is_super_admin'):
        if current_user.get('wecom_user_id'):
            # 强制过滤：只能看自己的客户
            where_clauses.append("owner_userid = ?")
            params.append(current_user['wecom_user_id'])
        else:
            # 没有绑定企微，返回空
            conn.close()
            return {
...
async def get_employees(request: Request, token: str = Depends(check_token)):
    """获取员工列表（企微通讯录，含统计数据）"""
    # 获取当前用户
    current_user = getattr(request.state, 'user', None)
    ...
    # 如果是超级管理员，或者使用旧API Token（向后兼容），显示所有员工
    # 如果是普通员工，只显示自己
    if not current_user:
        # 使用旧的 api_token，向后兼容，显示所有数据
        cursor.execute("SELECT * FROM employees_contacts")
    elif current_user.get('is_super_admin'):
        # 超级管理员，显示所有员工
        cursor.execute("SELECT * FROM employees_contacts")
    elif current_user.get('wecom_user_id'):
        # 普通员工只能看到自己
        cursor.execute("SELECT * FROM employees_contacts WHERE id = ?", (current_user['wecom_user_id'],))
    else:
        # 没有企微绑定的账号，返回空
        conn.close()
...
async def sync_employees(request: SyncRequest, token: str = Depends(check_token)):
    ...
    try:
        ...
        for user in users:
            ...
            cursor.execute("SELECT id FROM employees_contacts WHERE id = ?", (userid,))
            ...
            if exists:
                ...
                cursor.execute("""
                    INSERT INTO employees_contacts (
...
async def get_tag_stats(request: Request, token: str = Depends(check_token)):
    """
    获取标签统计数据 - 修复版
    支持数据隔离：超级管理员看全部，普通员工只看自己的客户
    """
    # 获取当前用户
    current_user = getattr(request.state, 'user', None)
        ...
        if current_user and not current_user.get('is_super_admin'):
            if current_user.get('wecom_user_id'):
                ...
                cursor.execute("""
                    ...
                    FROM customers 
                    WHERE owner_userid = ?
                    AND enterprise_tags IS NOT NULL AND enterprise_tags != '' AND enterprise_tags != '[]'
                """, (current_user['wecom_user_id'],))
            else:
                # 没有绑定企微，返回空统计
                conn.close()
                return {
...
async def get_province_stats(request: Request, token: str = Depends(check_token)):
    """
    获取省份统计数据（只显示有人的省份）
    支持数据隔离：超级管理员看全部，普通员工只看自己的客户
    """

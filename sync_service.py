...
class SyncService:
    ...
    def _sync_incremental_customers(self, task_id: str):
        ...
        try:
            ...
            # 设置初始状态（让前端知道正在处理）
            self._update_task(task_id, status='running', total_count=1)
            ...
            for user in users:
                ...
                if processed_users % 10 == 0:
                    print(f"📋 正在扫描员工客户列表: {processed_users}/{len(users)} ({processed_users*100//len(users)}%)")
                
                for eid in external_userids:
                    # 优化后的增量同步逻辑：
                    # 1. 如果客户在"需要更新列表"中 -> 同步（旧客户需要更新）
                    # 2. 如果客户不在数据库中 -> 同步（新客户）
                    # 3. 否则 -> 跳过（客户最近已同步过）
                    
                    if eid in db_customers_to_update:
                        # 旧客户，但需要更新（超过6小时未同步）

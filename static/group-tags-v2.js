...
(function() {
    ...
    // 模块变量
    if (typeof window._groupTagsList === 'undefined') {
        window._groupTagsList = [];
    }
    if (typeof window._currentEditingGroupId === 'undefined') {
        window._currentEditingGroupId = null;
    }
    if (typeof window._currentEditingTagId === 'undefined') {
        window._currentEditingTagId = null;
    }
...
function closeGroupTagsSyncModal() {
    ...
    _currentGroupTagsSyncTaskId = null;
...
async function stopGroupTagsSyncTask() {
    if (!_currentGroupTagsSyncTaskId) {
    ...
    try {
        const res = await fetch(`/api/sync/group-tags/stop/${_currentGroupTagsSyncTaskId}?api_token=${apiToken}`, {
            method: 'POST'
        });

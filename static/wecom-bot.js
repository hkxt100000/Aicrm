...
function showAddWebhookDialog(groupType) {
    ...
    modal.innerHTML = `
        <div style="background: white; border-radius: 12px; width: 90%; max-width: 600px; max-height: 90vh; overflow-y: auto; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15);">
            ...
            <div style="padding: 16px 24px; border-top: 1px solid rgba(145, 158, 171, 0.12); display: flex; justify-content: flex-end; gap: 12px;">
                <button onclick="closeAddWebhookDialog()" 
                ...
                <button onclick="saveWebhook('${groupType}')" 
...
async function saveWebhook(groupType) {
    ...
    try {
        const response = await fetch('/api/bot/webhooks', {
            method: 'POST',
            headers: {
...
async function loadWebhooks(groupType) {
    ...
    try {
        const url = `/api/bot/webhooks?group_type=${groupType}`;
        ...
        console.log('[Webhook] 原始数据:', webhooks);
        console.log('[Webhook] 数据类型:', typeof webhooks, '是否数组:', Array.isArray(webhooks));
...
function renderWebhookList(groupType, webhooks) {
    ...
    webhooks.forEach(webhook => {
        const statusBadge = webhook.status === 'active' 
            ? '<span style="padding: 4px 12px; background: rgba(0, 167, 111, 0.12); color: #005249; border-radius: 4px; font-size: 12px; font-weight: 500;"><i class="fas fa-check-circle"></i> 正常</span>'
            : '<span style="padding: 4px 12px; background: rgba(255, 86, 48, 0.12); color: #7A0C2E; border-radius: 4px; font-size: 12px; font-weight: 500;"><i class="fas fa-times-circle"></i> 停用</span>';
        
        html += `
            <div style="border: 1px solid rgba(145, 158, 171, 0.12); border-radius: 12px; padding: 20px; background: white; transition: all 0.2s;"
                onmouseover="this.style.boxShadow='0 4px 12px rgba(145, 158, 171, 0.12)'; this.style.borderColor='#FF6B2C'" 
                onmouseout="this.style.boxShadow='none'; this.style.borderColor='rgba(145, 158, 171, 0.12)'">
                
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 16px;">
                    <div style="flex: 1;">
                        <h4 style="margin: 0 0 4px 0; font-size: 16px; font-weight: 600; color: #212B36;">
                            <i class="fas fa-comments" style="color: #FF6B2C; margin-right: 6px;"></i>
                            ${webhook.group_name}
                        </h4>
                        ${webhook.purpose ? `<p style="margin: 4px 0; font-size: 13px; color: #637381;">${webhook.purpose}</p>` : ''}
                    </div>
                    <div>${statusBadge}</div>
                </div>
                
                <div style="padding: 12px; background: #F9FAFB; border-radius: 6px; margin-bottom: 16px;">
                    <p style="margin: 0; font-size: 12px; color: #637381; word-break: break-all; font-family: 'Courier New', monospace;">
                        ${webhook.webhook_url.substring(0, 60)}...
                    </p>
                </div>
                
                ${webhook.remark ? `
                    <div style="padding: 12px; background: rgba(255, 171, 0, 0.08); border-left: 3px solid #FFAB00; border-radius: 4px; margin-bottom: 16px;">
                        <p style="margin: 0; font-size: 13px; color: #7A4F01;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 12px; border-top: 1px solid rgba(145, 158, 171, 0.08);">
                    <span style="font-size: 12px; color: #919EAB;">
                        <i class="far fa-clock"></i>
                        ${new Date(webhook.created_at).toLocaleString('zh-CN', {month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit'})}
                    </span>
                    <div style="display: flex; gap: 8px;">
                        <button onclick="editWebhook(${webhook.id}, '${groupType}')" 
                        ...
                        <button onclick="toggleWebhookStatus(${webhook.id}, '${webhook.status}', '${groupType}')" 
                            style="padding: 6px 12px; border: 1px solid rgba(145, 158, 171, 0.24); border-radius: 6px; background: white; color: ${webhook.status === 'active' ? '#FF9800' : '#00A76F'}; font-size: 12px; cursor: pointer; transition: all 0.2s;"
                            onmouseover="this.style.background='#F4F6F8'" 
                            onmouseout="this.style.background='white'">
                            <i class="fas fa-${webhook.status === 'active' ? 'pause' : 'play'}"></i> ${webhook.status === 'active' ? '停用' : '启用'}
                        </button>
                        <button onclick="deleteWebhook(${webhook.id}, '${groupType}')" 
                            style="padding: 6px 12px; border: 1px solid rgba(255, 86, 48, 0.24); border-radius: 6px; background: white; color: #FF5630; font-size: 12px; cursor: pointer; transition: all 0.2s;"
                            onmouseover="this.style.background='rgba(255, 86, 48, 0.08)'" 
                            onmouseout="this.style.background='white'">
...
async function testWebhook(webhookId) {
    ...
    try {
        const response = await fetch(`/api/bot/webhooks/${webhookId}/test`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
...
async function toggleWebhookStatus(webhookId, currentStatus, groupType) {
    try {
        ...
        const response = await fetch(`/api/bot/webhooks/${webhookId}/toggle`, {
            method: 'POST'
        });
...
async function sendNotification(groupType) {
    ...
    // 发送请求
    try {
        const response = await fetch('/api/bot/notifications', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function loadNotifications(groupType) {
    try {
        const response = await fetch(`/api/bot/notifications?group_type=${groupType}&limit=50`);
        if (!response.ok) throw new Error('加载失败');
...
async function viewNotificationDetail(notificationId) {
    try {
        const response = await fetch(`/api/bot/notifications/${notificationId}`);

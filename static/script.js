...
function initApp() {
    ...
    // 导航切换（排除导航组的 toggle 按钮）
    const navItems = document.querySelectorAll('.nav-item:not(.nav-group-toggle)');
    console.log('[初始化] 找到导航项数量:', navItems.length);
    
    navItems.forEach((item, index) => {
        console.log(`[初始化] 绑定导航项 ${index}:`, item.dataset.module, item.textContent.trim());
        item.addEventListener('click', (e) => {
            e.preventDefault();
            const module = item.dataset.module;
            console.log('[导航点击]', module);
            if (module) {  // 只有有 data-module 的才切换
                switchModule(module);
            }
        });
    });
...
if (document.readyState === 'loading') {
    ...
    console.log('[script.js] DOM 还在加载，等待 DOMContentLoaded');
    document.addEventListener('DOMContentLoaded', initApp);
} else {
    // DOM 已经加载完成，直接执行
    console.log('[script.js] DOM 已加载完成，立即执行初始化');
    initApp();
}
...
function switchModule(moduleName) {
    console.log('[切换模块] 开始:', moduleName);
    console.log('[切换模块] 当前所有模块:', Array.from(document.querySelectorAll('.module')).map(m => m.id));
    ...
    document.querySelectorAll('.nav-item:not(.nav-group-toggle)').forEach(item => {
        ...
        if (item.dataset.module === moduleName) {
            ...
            console.log('[切换模块] 高亮导航项:', item.textContent.trim());
        }
    });
    ...
    } else if (moduleName === 'supplier-notify') {
        console.log('[加载供应商通知群]');
        setTimeout(() => {
            if (typeof window.loadWebhooks === 'function') {
                window.loadWebhooks('supplier');
                window.loadNotifications('supplier');
            } else {
                console.error('[错误] loadWebhooks 函数未找到');
            }
        }, 100);
    } else if (moduleName === 'agent-notify') {
        console.log('[加载代理商通知群]');
        setTimeout(() => {
            if (typeof window.loadWebhooks === 'function') {
                window.loadWebhooks('agent');
                window.loadNotifications('agent');
            } else {
                console.error('[错误] loadWebhooks 函数未找到');
            }
        }, 100);
    } else if (moduleName === 'data-sources-internal') {
        console.log('[加载内部数据源]');
        
        // 等待 dataSourceManager 准备就绪
        const waitForDataSource = () => {
            if (window.dataSourceManagerReady && typeof dataSourceManager !== 'undefined') {
                console.log('[数据源] dataSourceManager 已就绪');
                if (!dataSourceManager.initialized) {
                    console.log('[数据源] 首次初始化');
                    dataSourceManager.initialized = true;
                    dataSourceManager.init().catch(err => {
                        console.error('[错误] 源数据管理初始化失败:', err);
                    });
                } else {
                    console.log('[数据源] 已初始化，刷新数据');
                    dataSourceManager.loadDataSources();
                }
            } else {
                console.log('[数据源] 等待 data-source.js 加载...');
                setTimeout(waitForDataSource, 50);
            }
        };
        
        waitForDataSource();
    } else if (moduleName === 'data-sources-external') {
        console.log('[加载外部数据源]');
        // 暂时没有额外逻辑
    } else if (moduleName === 'spreadsheet-internal') {
        console.log('[加载对内智能表格]');
        loadSpreadsheetList();
    } else if (moduleName === 'spreadsheet-external') {
        console.log('[加载对外智能表]');
        // 暂时没有额外逻辑
    } else if (moduleName === 'data-source-detail') {
        console.log('[加载数据源详情]');
        // 由 dataSourceManager.viewRecords() 处理
    } else if (moduleName === 'settings') {
        console.log('[加载系统设置]');
        loadWecomConfig();
    } else if (moduleName === 'employee-manage') {
        console.log('[加载员工管理]');
        // 员工管理模块由 employee-manage.js 处理
        if (typeof initEmployeeManage === 'function') {
            initEmployeeManage();
        }
    } else if (moduleName === 'permission-manage') {
        console.log('[加载权限管理]');
        // 权限管理模块由 permission-manage.js 处理
        if (typeof initPermissionManage === 'function') {
...
function renderCustomers(customers) {
    ...
    if (!customers || customers.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="9" style="text-align: center; padding: 60px 20px; color: var(--grey-500);">
            ...
            <tr>
                ...
                <td>
                    <div style="display: flex; flex-direction: column; gap: 4px;">
                        <span style="font-weight: 600; color: var(--grey-800); font-size: 14px;">${customer.name || '未知'}</span>
                        <span style="font-size: 12px; color: var(--grey-500);">${customer.remark || '暂无备注昵称'}</span>
                    </div>
                </td>
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 600px;">
            <div class="modal-header">
                ...
                <button class="modal-close" onclick="closeSyncProgressModal()" style="
                    position: absolute;
                    right: 20px;
                    top: 20px;
            ...
            <div class="spreadsheet-card" onclick="viewSpreadsheetDetail('${sheet.id}')">
                ...
                <div class="spreadsheet-footer">
                    ...
                    <div class="spreadsheet-actions" onclick="event.stopPropagation();">
                        ...
                        <button class="btn-icon btn-icon-danger" onclick="deleteSpreadsheet('${sheet.id}', '${sheet.name}')" title="删除表格">
...
// 删除表格
async function deleteSpreadsheet(spreadsheetId, spreadsheetName) {
    console.log('[删除表格] 调用删除函数', { spreadsheetId, spreadsheetName });
    
    if (!spreadsheetId) {
        console.error('[删除表格] spreadsheetId 为空');
        return;
    }
    
    if (!confirm(`确定要删除表格"${spreadsheetName}"吗？此操作不可恢复。`)) {
        console.log('[删除表格] 用户取消删除');
        return;
    }
    
    console.log('[删除表格] 开始发送删除请求');
    
    try {
        const url = `/api/spreadsheet/${spreadsheetId}?api_token=${apiToken}`;
        console.log('[删除表格] 请求URL:', url);
        
        const res = await fetch(url, {
            method: 'DELETE'
        });
        
        console.log('[删除表格] 响应状态:', res.status);
        
        const data = await res.json();
        console.log('[删除表格] 响应数据:', data);

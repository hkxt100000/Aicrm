...
def get_webhooks(group_type: Optional[str] = None, api_token: str = None):
    ...
    try:
        ...
        print(f"[Bot API] 获取webhook列表失败: {e}")
        import traceback
        traceback.print_exc()
        # 返回JSON而不是抛出异常，保持API响应格式一致
        return {'success': False, 'message': f'获取webhook列表失败: {str(e)}'}
...
def get_notifications(group_type: Optional[str] = None, status: Optional[str] = None, api_token: str = None):
    ...
    try:
        ...
        print(f"[Bot API] 获取通知列表失败: {e}")
        import traceback
        traceback.print_exc()
        # 返回JSON而不是抛出异常
        return {'success': False, 'message': f'获取通知列表失败: {str(e)}', 'data': []}
